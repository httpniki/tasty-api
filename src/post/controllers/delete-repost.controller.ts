...
export default class DeleteRepostController {
   ...
   private async execute() {
      ...
      try {
         await this.postService.deleteRepost(uuid, user_uuid)
      } catch (error: any) {
         ...
         if (error.name === 'post_not_reposted') {
            const exception = ExceptionFactory.invalidParam('Post is not reposted')
            return this.res.status(exception.status).json(exception.toJSON())
         }
